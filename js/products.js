document.addEventListener('DOMContentLoaded', async () => {
  const CATEGORIES = ['Electronics', 'Fashion', 'Shoes', 'Phones', 'Cars', 'Others'];
    ...
    if (queryStr) {
      ...
      summaryEl.innerHTML = `
        <div style="border-bottom: 2px solid #febd69; padding-bottom: 1rem; margin-bottom: 1.5rem;">
          ...
          <p class="text-muted mt-1">${allProducts.length} items found</p>
        </div>
      `;
    } else {
      summaryEl.innerHTML = `
        <div style="border-bottom: 2px solid #febd69; padding-bottom: 1rem; margin-bottom: 1.5rem;">
          ...
          <p class="text-muted mt-1">${allProducts.length} items available</p>
        </div>
      `;
    }
